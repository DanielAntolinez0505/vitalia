...
const SleepHistory: React.FC = () => {
  const averageSleepDuration =
    sleepData.reduce((acc, entry) => acc + entry.duration, 0) /
    sleepData.length;
  const goodSleepNights = sleepData.filter(
    (entry) => entry.quality === "Buena"
  ).length;
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-3xl font-bold">
                {averageSleepDuration.toFixed(2)} horas
              </p>
            </div>
            <div>
              <p className="text-lg font-semibold">Noches con buen sueño:</p>
              <p className="text-3xl font-bold">
                {goodSleepNights} de {sleepData.length}
              </p>
            </div>
          </div>
        </CardContent>
